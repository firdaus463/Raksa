...
@php
    $userName = $name ?? (auth()->user()->name ?? 'Admin EBMD');
...
<div x-data="{ open: false }" @click.outside="open = false" class="relative">
    ...
    >
        <div class="hidden text-right sm:block">
            ...
            <p class="text-[11px] text-raksa-neutral/80">{{ $role }}</p>
        </div>
    ...
    >
        ...
        <div class="py-1 space-y-0.5">
            <a href="{{ route('pengaturan.index') }}" class="flex items-center gap-2.5 rounded-xl px-3 py-2 text-xs font-semibold text-raksa-neutral hover:bg-raksa-surface hover:text-raksa-primary transition">
